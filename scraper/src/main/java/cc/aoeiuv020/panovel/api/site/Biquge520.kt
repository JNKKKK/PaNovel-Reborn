...
class Biquge520 : DslJsoupNovelContext() { init {
    site {
        name = "笔趣阁520"
        baseUrl = "https://www.biquge520.com"
    }
    ...
    chapters {
        ...
        val pageCount = firstPage.select("li").mapNotNull { li ->
            biquge520PaginationRegex.matchEntire(li.text().trim())?.groupValues?.get(2)?.toIntOrNull()
        }.firstOrNull() ?: 1
    ...
    content {
        ...
        val (paragraphs, totalPages) = parseBiquge520Page(firstDoc)
        ...
        if (totalPages > 1) {
            ...
            for (page in 2..totalPages) {
                ...
                val doc = parse(connect(pageUrl))
                val (pageParagraphs, _) = parseBiquge520Page(doc)
...
private val biquge520PaginationRegex = Regex("(\\d+)/(\\d+)")
private val biquge520PageIndicatorRegex = Regex("第\\(\\d+/\\d+\\)页")
private val biquge520TotalPagesRegex = Regex("第\\(\\d+/(\\d+)\\)页")

private fun parseBiquge520Page(doc: org.jsoup.nodes.Document): Pair<List<String>, Int> {
    val article = doc.selectFirst("article") ?: return Pair(emptyList(), 1)
    val html = article.html()
    val totalPages = biquge520TotalPagesRegex.find(html)?.groupValues?.get(1)?.toIntOrNull() ?: 1
    val lines = html.split(Regex("<br\\s*/?>"))
        .map { it.replace("&nbsp;", " ").replace(Regex("<[^>]+>"), "").trim() }
        .filter { it.isNotEmpty() && !biquge520PageIndicatorRegex.containsMatchIn(it) }
