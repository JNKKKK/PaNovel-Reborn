...
class Biquji : DslJsoupNovelContext() { init {
    site {
        name = "笔趣集"
        baseUrl = "https://www.biquji.com"
    }
    ...
    chapters {
        ...
        val pageCount = firstPage.select("li").mapNotNull { li ->
            biqujiPaginationRegex.matchEntire(li.text().trim())?.groupValues?.get(2)?.toIntOrNull()
        }.firstOrNull() ?: 1
...
private val biqujiPaginationRegex = Regex("(\\d+)/(\\d+)")
