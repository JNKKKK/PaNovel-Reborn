...
class Bpshu : DslJsoupNovelContext() { init {
    site {
        name = "笔趣书"
        baseUrl = "https://www.bpshu.cc"
    }
    bookIdRegex = "/(\\d+)"
    bookIdWithChapterIdRegex = "/(\\d+/\\d+)"
    ...
    chapters {
        ...
        val pageCount = firstPage.select("li").mapNotNull { li ->
            bpshuPaginationRegex.matchEntire(li.text().trim())?.groupValues?.get(2)?.toIntOrNull()
        }.firstOrNull() ?: 1
...
private val bpshuPaginationRegex = Regex("(\\d+)/(\\d+)")
