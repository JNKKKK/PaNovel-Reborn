...
class Sbiquge : DslJsoupNovelContext() { init {
    site {
        name = "笔趣阁s"
        baseUrl = "https://www.sbiquge.com"
    }
    bookIdRegex = "/(\\d+_\\d+)"
    ...
    chapters {
        ...
        val pageCount = firstPage.select("li").mapNotNull { li ->
            sbiqugePaginationRegex.matchEntire(li.text().trim())?.groupValues?.get(2)?.toIntOrNull()
        }.firstOrNull() ?: 1
...
private val sbiqugePaginationRegex = Regex("(\\d+)/(\\d+)")
