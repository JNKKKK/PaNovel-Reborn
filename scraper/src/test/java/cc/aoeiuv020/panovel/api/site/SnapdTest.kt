...
class SnapdTest {
    ...
    @Test
    fun testDetail() {
        val detail = context.getNovelDetail("29583")
        assertEquals("我在大周捉妖怪", detail.novel.name)
        assertEquals("夜入白", detail.novel.author)
    ...
    @Test
    fun testDetailFromUrl() {
        val detail = context.getNovelDetail("https://www.snapd.net/read/201/")
        assertEquals("我在大周捉妖怪", detail.novel.name)
        assertEquals("夜入白", detail.novel.author)
        println("detail from URL: ${detail.novel.name}")
    }

    @Test
    fun testChapters() {
        val chapters = context.getNovelChaptersAsc("29583")
    ...
    @Test
    fun testContent() {
        val content = context.getNovelContent("29583/1")
